      ...
      if (productPerMonthDbObj[idMonthKey]) {
        ...
        bulkUpdateArray.push({
          updateOne: {
            ...
            update: { $set: { [`quantityPerDayObj.${dayString}`]: requiredQuantity } },
            upsert: true,
          },
        });
      }
...
const checkAvalibltyForPurchesProducts = (productsFromDbObj, productsArray) => {
  const bulkUpdateArray = [];
  productsArray.forEach((purchaseProduct) => {
    const { productId } = purchaseProduct;
    const productFromDb = productsFromDbObj[productId.toString()];
    if (productFromDb.stock.quantity - purchaseProduct.quantity < 0) {
      throw new ApiError(httpStatus.BAD_REQUEST, 'Product quantity exceeds stock availablity');
    }
    bulkUpdateArray.push({
      updateOne: {
...
const createOrder = async (orderBody) => {
  ...
  try {
    ...
    const consumePurches = checkAvalibltyForPurchesProducts(productsFromDbObj, purchaseProducts);
    const consumeHire = checkAvalibltyForHireProducts(productsFromDbObj, productPerMonthDbObj, hireProducts);

    await Product.bulkWrite(consumePurches, { session });
    await Calendar.bulkWrite(consumeHire, { session });
